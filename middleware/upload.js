...
const generateUploadFilename = async (req, file) => {
    ...
    try {
        ...
        } else if (req.originalUrl.includes('/teams/logo')) {
...
if (process.env.IMGBB_API_KEY && imgbbUploader) {
    upload = multer({ storage: new ImgbbStorage({ apiKey: process.env.IMGBB_API_KEY }), limits: { fileSize: 5 * 1024 * 1024 } });
} else if (process.env.CLOUDINARY_URL) {
    const cloudStorage = new CloudinaryStorage({
        cloudinary: cloudinary,
        params: {
            folder: 'valorant-tourney', allowed_formats: ['jpg', 'png', 'jpeg', 'gif'],
